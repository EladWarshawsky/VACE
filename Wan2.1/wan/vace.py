...
class WanVace(WanT2V):
    ...
    def generate(self,
        ...
        # Determine device type for backend-agnostic autocast
        device_type = self.device.type

        # evaluation mode with nested, backend-agnostic context managers
        with torch.no_grad(), \
     torch.autocast(device_type=device_type, dtype=self.param_dtype, enabled=(device_type in ['cuda', 'mps'])), \
     no_sync():
